...
inline void MulElementwise(int size, const ArithmeticParams& params,
  ...
  for (; i < size;) {
    ...
    vint32m4_t product = __riscv_vwmul_vv_i32m4(v1_16, v2_16, vl);
    if (left_shift > 0) {
    ...
    product =
        __riscv_vmin_vx_i32m4(product, params.quantized_activation_max, vl);
    product =
        __riscv_vmax_vx_i32m4(product, params.quantized_activation_min, vl);
    vint16m2_t narrowed16 =
        __riscv_vnclip_wx_i16m2(product, 0, __RISCV_VXRM_RDN, vl);
    vint8m1_t result =
...
inline void MulSimpleBroadcast(int size, const ArithmeticParams& params,
  ...
  for (; i < size;) {
    ...
    v2_16 = __riscv_vadd_vx_i16m2(v2_16, params.input2_offset, vl);
    vint32m4_t product =
        __riscv_vwmul_vx_i32m4(v2_16, input1_val, vl);
    if (left_shift > 0) {
      product = __riscv_vsll_vx_i32m4(product, left_shift, vl);
    }
    product = rvv_utils::SaturatingRoundingDoublingHighMul_m4(
